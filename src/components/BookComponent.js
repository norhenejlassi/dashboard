...
class BookComponent extends React.Component{
    ...
    getBooksByPagination(currentPage){
        ...
        axios.get("http://localhost:8006/api/test/books?page="+currentPage+"&size="+this.state.recordPerPage)
    ...
    searchBook=(currentPage)=> {
        currentPage=currentPage-1;
        axios.get("http://localhost:8888/books/"+this.state.search+"?page="+currentPage+"&size="+this.state.recordPerPage)
        .then(response => response.data).then((data) =>{
             this.setState({books:data.content,
                totalPages:data.totalPages,
                totalElements: data.totalElements,
